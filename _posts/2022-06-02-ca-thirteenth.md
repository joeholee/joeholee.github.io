...
ADDVS.D: add vector to a scalar 스칼라에 
...
메모리 접근이 많아서 메모리의 지원이 필요하다. memory banking을 써도 여전히 계산에 비해 메모리가 부족하다. 원하지 않는 모양으로 데이터가 맵핑이 다르게 되어있어도 곤란하다.

**scatter-gather**

떨어져있는 메모리들을 모아준다(메모리에 strided가 되어있지 않으면 모아준다)

메모리의 위치를 인덱스 처리하는 느낌인듯?
